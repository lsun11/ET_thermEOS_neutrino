subroutine poisson_solver_fluid_sphcoord(sou, pot)
  ...
  use grid_parameter, only : nrf, npg, ntg, nlg
  ...
  use weight_midpoint_fluid_sphcoord, only : hwrtpg_fc
  use make_array_3d
  implicit none
! 
...
! 
! 
  call alloc_array3d(work1c, 1, ntg, 1, nrf, 0, nlg)
  call alloc_array3d(work1s, 1, ntg, 1, nrf, 0, nlg)
  call alloc_array3d(work2c, 1, nrf, 0, nlg, 0, nlg)
  call alloc_array3d(work2s, 1, nrf, 0, nlg, 0, nlg)
  call alloc_array3d(work3c, 0, nlg, 0, nlg, 0, nrf)
  call alloc_array3d(work3s, 0, nlg, 0, nlg, 0, nrf)
  call alloc_array3d(work4c, 0, nlg, 0, nrf, 0, ntg)
  call alloc_array3d(work4s, 0, nlg, 0, nrf, 0, ntg)
!
  work1c = 0.0
  work1s = 0.0
  do im = 0, nlg
    do irr = 1, nrf
      do itt = 1, ntg
        ...
        + hwrtpg_fc(irr,itt,ipp)*sou(irr,itt,ipp)*hcosmpg(im,ipp)
          work1s(itt,irr,im) = work1s(itt,irr,im) & 
        + hwrtpg_fc(irr,itt,ipp)*sou(irr,itt,ipp)*hsinmpg(im,ipp)
        end do
      end do
    end do
  end do
