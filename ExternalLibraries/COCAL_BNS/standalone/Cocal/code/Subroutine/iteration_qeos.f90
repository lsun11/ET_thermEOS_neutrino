subroutine iteration_qeos(iter_count)
  ...
  use make_array_4d
  use def_metric
  use def_matter    ! include emd(:,:,:), we need def_matter_quark
...
!  use interface_sourceterm_trG_peos
  use interface_source_HaC_CF_qeos
  use interface_source_trG_CF_qeos
  use interface_source_MoC_CF_qeos
  use interface_sourceterm_MoC_peos
  use interface_compute_shift
  use interface_compute_alps2alph
  use interface_hydrostatic_eq_qeos
  use interface_calc_surface_qeos
  use interface_update_grfield
  use interface_update_matter
  use interface_update_parameter_qeos
  ...
  real(long), pointer :: potf(:,:,:), rhof_bak(:,:,:)
...
!
  ...
  call alloc_array3d(rhof_bak,0,nrf,0,ntf,0,npf)
...
!!    call sourceterm_HaC_peos(sou)
    call source_HaC_CF_qeos(sou)
    call poisson_solver(sou,pot)
    pot = pot + 1.0d0   ! 1 is the surface integral at infinity from the green's function
    call update_grfield(pot,psi,conv_gra)
!!    call reset_outer_boundary_RNS_CF('psi')
    call update_parameter_qeos(conv_den)
! --
!
!!    call sourceterm_trG_peos(sou)
    call source_trG_CF_qeos(sou)
    call poisson_solver(sou,pot)
    pot = pot + 1.0d0
...
!!    call sourceterm_MoC_peos(souvec,sou)
    call source_MoC_CF_qeos(souvec)
...
!
    ...
    write(6,*)  "====>", flag_bvxd, flag_bvyd, flag_bvzd
!
! -- Hydro equations.
if (error_rhof_tmp.le.eps*0.1d0.and.error_metric_tmp.gt.eps*2.0d0.or. (chrot=='p' .and. chgra=='e' .and. chope=='r')) then 
  write(6,*)' ## SKIP HYDRO ##' 
else
  do ihy = 1, hydro_iter
    call interpo_gr2fl_metric_CF
    call rotation_law
    call hydrostatic_eq_qeos(potf)
!testtest    call calc_surface_qeos(potrs, potf)
    rhof_bak = rhof
    call update_matter(potf,rhof,conv_den)
    call calc_surface_qeos(potrs, rhof)
...
!    call calc_vector_phi_matter(1)
!
    call error_matter_type2(rhof,rhof_bak,error_rhof,flag)
    call error_surface(rs,rs_bak,error_rs,flag_rs)
    call printout_error(iter_count,error_rhof)
    call printout_error_matter('rhof ',iter_count,error_rhof)
    call printout_error_matter('rs  ',iter_count,error_rs)
    call error_monitor_matter(rhof,rhof_bak,'rhof ',2,ntf/2,2)
...
!!    &   (flag_bvxd==0).and.(flag_bvyd==0).and.(flag_bvzd==0)) exit
    if (chrot=='p' .and. chgra=='e' .and. chope=='r') then
      if ( (flag_bvxd==0).and.(flag_bvyd==0).and.(flag_bvzd==0) )  exit
    else
      if (flag == 0) exit
    end if
