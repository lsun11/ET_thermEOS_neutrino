subroutine poisson_solver_At_homosol_lesq(pot_bc,pot_nb,pot_surf,char_sym)
  ...
  integer    :: m_max, mm_max
  integer    :: ii, jj
  character(len=6) :: char_sym
!
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
! Coefficients of homogeneous solution is computed.
! For axisymmetric case (char_sym.eq.'axisym') m is up to 2.
! - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
!
  ...
  nbou = (nlg+1)*(nlg+1)
  if (char_sym.eq.'axisym') nbou = 5*nlg-1
...
!
  ...
  do il = 0, nlg
    m_max = il
    if (char_sym.eq.'axisym') m_max = min0(2,il)
    do im = 0, m_max
      ieqs = ieqs + 1
!
      do ip = 1, npg
...
!
          ...
          do ill = 0, nlg
            mm_max = ill
            if (char_sym.eq.'axisym') mm_max = min0(2,ill)
            do imm = 0, mm_max
              ivec = ivec + 1
