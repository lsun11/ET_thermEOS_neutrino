...
!     to the potential pot from a surface term at the surface ir = nrtot 
!     The integrand is proportional to a function 
!      psis = partial_r pot(R,theta,phi)n_l(R) - pot \partial_r n_l(omega R)
!     The routine is adapted from grav4, omitting the r integral, 
!     but retaining the theta and phi integrals and the corresponding sums
!
! --  Note: the surface integral is over the solid angle dO, NOT R^2 dO.
! --  The Factor R^2 is included in the radial Green's function.
!
...
! 
  ...
  pot(:,:,:) = pi4inv*pot(:,:,:)
!!!  rsqpi4inv =rg(nrg)*rg(nrg)*pi4inv
!!!  pot(:,:,:) = - rsqpi4inv*pot(:,:,:)
!
  deallocate( work11c)
  deallocate(gwork11c)
