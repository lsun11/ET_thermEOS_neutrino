...
!
      ...
      do jj = 1, 3
        do ii = 1, 3
          ...
          if (jj.eq.2) then 
            dovufcjj = vphi(ia,jj)          
            doAufcjj = (-MHDfnc_dAt_val(ia)/ome)*vphi(ia,jj)
          end if
          gmdovoA(ia) = gmdovoA(ia) &
          &           + gm(ia,ii,jj)*dovufcii*oAufc(ia,jj) &
          &           + gm(ia,ii,jj)*ovufc(ia,ii)*doAufcjj
...
!
      dterm1do(ia) = - psi2al(ia)**(2.d0*radi**2)*2.0d0*gmdovoA(ia)
