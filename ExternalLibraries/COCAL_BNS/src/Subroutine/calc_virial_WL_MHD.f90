subroutine calc_virial_WL_MHD
  use phys_constant, only  : long, pi
  use grid_parameter, only : nrg, ntg, npg, nrf, ntf, npf
  use def_matter_parameter, only : radi
  use def_quantities, only : T_kinene, P_intene, M_emfene, W_gravene, &
  &                          ToverW, PoverW, MoverW, Virial
  use make_array_3d
  use interface_source_virial_WL_MHD
...
!
  ...
  call alloc_array3d(sou_Memf, 0, nrg, 0, ntg, 0, npg)
  call alloc_array3d(sou_Wgra, 0, nrg, 0, ntg, 0, npg)
!
  call source_virial_WL_MHD(sou_Tkin,sou_Pint,sou_Memf,sou_Wgra)
!
...
!
  call vol_int_grav(sou_Memf,volg)
  M_emfene = radi*volg
!
...
!
  ...
  MoverW = M_emfene/dabs(W_gravene)
  Virial = (2.0d0*T_kinene + 3.0d0*P_intene + M_emfene + W_gravene)/W_gravene
  Virial = dabs(Virial)
...
!
  ...
  deallocate(sou_Wgra)
!
end subroutine calc_virial_WL_MHD
