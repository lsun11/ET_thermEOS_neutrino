subroutine update_parameter_BNS_peos_lecc_irrot(convf)
  use phys_constant, only  : long
  use grid_parameter, only : nrg, ntg, npg, nrf_deform, nrf, ntgxy, npgxzm
  use def_metric, only : psi, alph, bvxd, bvyd, bvzd, tfkij, tfkijkij, alps
...
!    lamout = ber + (bxout+0)*dxvepout + ovout*dyvepout + (bzout+0)*dzvepout
    ...
    lamin  = ber + ovin*dyvepin   + velx*dxvepin
    lammid = ber + ovmid*dyvepmid + velx*dxvepmid
    lamout = ber + ovout*dyvepout + velx*dxvepout
...
!
    ...
    if(term4in<0 .or. term4out<0 .or. term4mid<0) then
      ...
      sgg(1) = -( -dlog(lamin)  + radi**2*ain   + 0.5d0*dlog(term4in))

      sgg(2) = -( -dlog(lamout) + radi**2*aout  + 0.5d0*dlog(term4out))

      sgg(3) = -( -dlog(lammid) + radi**2*amid  + 0.5d0*dlog(term4mid) + loghc)
    end if
!
    term6in  = dvep2in*2.0d0*radi*dlog(pm4in) 

    term6mid = dvep2mid*2.0d0*radi*dlog(pm4mid)

    term6out = dvep2out*2.0d0*radi*dlog(pm4out)

!  ain is ln(\alpha)
    gg(1,1) = vphidvepin*(-1.0d0 )/lamin

    gg(1,2) = (-1.0d0 )/lamin

    gg(1,3) = 2.0d0*radi*ain + term6in/term4in/(2.0d0*1.0d0)

    gg(2,1) = vphidvepout*(-1.0d0 )/lamout

    gg(2,2) = (-1.0d0 )/lamout

    gg(2,3) = 2.0d0*radi*aout + term6out/term4out/(2.0d0*1.0d0)

    gg(3,1) = vphidvepmid*(-1.0d0 )/lammid

    gg(3,2) = (-1.0d0 )/lammid

    gg(3,3) = 2.0d0*radi*amid + term6mid/term4mid/(2.0d0*hc*hc)
!
!    test
...
!
!  stop
end subroutine update_parameter_BNS_peos_lecc_irrot
