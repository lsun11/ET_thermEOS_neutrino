subroutine sourceterm_MoC_WL_all_bhex(souvec)
  ...
  use def_formulation, only : swflu, shift_scale
...
!
  ...
  real(long), pointer :: grad4x(:,:,:), grad4y(:,:,:), grad4z(:,:,:)
  real(long), pointer :: fnc2(:,:,:),   fnc4(:,:,:),divBeta(:,:,:)
  real(long), pointer :: dabfnc(:,:)
  real(long), pointer :: cri1be(:,:,:), cri2be(:,:,:), cri3be(:,:,:), &
              &          cri4be(:,:,:), cri5be(:,:,:), cri6be(:,:,:)
  real(long) :: gmxxu, gmxyu, gmxzu, gmyyu, gmyzu, gmzzu, &
  &          gmyxu, gmzxu, gmzyu, &
  ...
  &          grad(1:3), ene, fnc2gc, &
  &          bvxdgc, bvydgc, bvzdgc, alpgc, psigc, &
  ...
  do ipg = 0, npg
    do itg = 0, ntg
      do irg = 0, nrg
        ...
        fnc2(irg,itg,ipg) = psigc**6/alpgc
!        call grgrad_gridpoint_type0(bvxd,dbxdx,dbxdy,dbxdz,irg,itg,ipg)
!        call grgrad_gridpoint_type0(bvyd,dbydx,dbydy,dbydz,irg,itg,ipg)
!        call grgrad_gridpoint_type0(bvzd,dbzdx,dbzdy,dbzdz,irg,itg,ipg)
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        hhxxu=hxxu(irg,itg,ipg) + bbxx
        hhxyu=hxyu(irg,itg,ipg) + bbxy
        hhxzu=hxzu(irg,itg,ipg) + bbxz
        hhyyu=hyyu(irg,itg,ipg) + bbyy
        hhyzu=hyzu(irg,itg,ipg) + bbyz
        hhzzu=hzzu(irg,itg,ipg) + bbzz
        hhyxu = hhxyu
        hhzxu = hhxzu
        hhzyu = hhyzu
!
        call grgrad_4th_gridpoint_bhex(bvxd,pdbvxdx,pdbvxdy,pdbvxdz,irg,itg,ipg)
        call grgrad_4th_gridpoint_bhex(bvyd,pdbvydx,pdbvydy,pdbvydz,irg,itg,ipg)
        call grgrad_4th_gridpoint_bhex(bvzd,pdbvzdx,pdbvzdy,pdbvzdz,irg,itg,ipg)
!
        divBeta(irg,itg,ipg) = pdbvxdx + pdbvydy + pdbvzdz 
!
...
!
        fnc4(irg,itg,ipg) = fnc4(irg,itg,ipg)                           &
                    &     - bvxd(irg,itg,ipg)*gmcrix_grid(irg,itg,ipg)  &
                    &     - bvyd(irg,itg,ipg)*gmcriy_grid(irg,itg,ipg)  &
                    &     - bvzd(irg,itg,ipg)*gmcriz_grid(irg,itg,ipg)
...
!
  do ipg = 1, npg
    do itg = 1, ntg
      do irg = 1, nrg
        call grgrad_midpoint_type0(divBeta,dxafn,dyafn,dzafn,irg,itg,ipg)
...
!
          ...
          bbxx = ps4oal2*bvxgc*bvxgc
          bbxy = ps4oal2*bvxgc*bvygc
          bbxz = ps4oal2*bvxgc*bvzgc
          bbyy = ps4oal2*bvygc*bvygc
          bbyz = ps4oal2*bvygc*bvzgc
          bbzz = ps4oal2*bvzgc*bvzgc
...
!            if(ii == 3) call dadbscalar_type0(bvzd,dabfnc,irg,itg,ipg)
            if(ii == 1) call dadbscalar_type3_bhex(bvxd,dabfnc,irg,itg,ipg)
            if(ii == 2) call dadbscalar_type3_bhex(bvyd,dabfnc,irg,itg,ipg)
            if(ii == 3) call dadbscalar_type3_bhex(bvzd,dabfnc,irg,itg,ipg)
...
!
          ...
          end if        !  waveless or IWM
!
          souvec(irg,itg,ipg,ii) = - san*ddivBeta(irg,itg,ipg,ii)            &
          &    - hddb + gdcb + gcrdb1 + gcrdb2 - san*gradfnc4                &
          &    -(rax*bvxgc + ray*bvygc + raz*bvzgc) - ome*divlp*cutoff       &
          &    - 2.0d0*afnc2inv*(  tfkax*dxafn + tfkay*dyafn + tfkaz*dzafn   &
          ...
          &    + 4.0d0*alpgc*dtrk(irg,itg,ipg,ii)/3.0d0
!
        end do
      end do
    end do
