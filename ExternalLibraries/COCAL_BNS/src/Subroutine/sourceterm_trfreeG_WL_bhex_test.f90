subroutine sourceterm_trfreeG_WL_bhex(souten)
  ...
  use def_metric
  use def_metric_excurve_grid, only : Lij_grid, tfkij_grid
  ...
  use def_kerr_schild, only : tfkij_grid_ks
  use def_formulation, only : swlp, swls, iswl, chgra, chope, se
...
!          if (chgra == 'k') tfliep4aij = tfaij*bepadpsi + ps4oal*tfliebaij*zerofac     !test1a
!          if (chgra == 'k') tfliep4aij = tfaij*bepadpsi*zerofac + ps4oal*tfliebaij     !test1b
