...
! write (6,*) "test values"
  write (6,'(a22, 1p,3e20.12)') " Old ome, ber, radi = ", ome, ber, radi
...
! write (6,'(1p,3e12.4)') pain, pamid, paout
  ...
  &   "ff=-Log[ber]+(",ain*radi*radi,")+0.5*Log[1.0-(",pain**(2*radi*radi),")*((",bin,")+ome*(",vphiyin,"))^2];"
  write(6,'(a3,f10.7,a11,f10.7,a19,f10.7,a12,f10.7,a7,f10.7,a6)') "gg=",loghc,    &
  &   "-Log[ber]+(",amid*radi*radi,")+0.5*Log[1.0-(",pamid**(2*radi*radi),")*((",bmid,")+ome*(",vphiymid,"))^2];"
!  write(6,'(a14,f10.7,a19,f10.7,a12,f10.7,a7,f10.7,a6)') &
...
!  ain is ln(\alpha)
    ...
    gg(1,2) = - 1.0d0/ber
!    gg(1,3) = 2.0d0*radi*ain + 0.5d0*dtermdrin/termin       ! d/dR is zero
!
!    gg(2,1) =   0.5d0*dtermdoout/termout                    ! this point does not exist
!    gg(2,2) = - 1.0d0/ber                                   ! this point does not exist 
!    gg(2,3) = 2.0d0*radi*aout + 0.5d0*dtermdrout/termout    ! d/dR is zero
!
    gg(2,1) =   0.5d0*dtermdomid/termmid
    gg(2,2) = - 1.0d0/ber
!    gg(3,3) = 2.0d0*radi*amid + 0.5d0*dtermdrmid/termmid    ! d/dR is zero
!
!    test
...
!    error = dmax1(dabs(ddome/ome),dabs(ddber/ber),dabs(ddradi/radi))
    error = dmax1(dabs(ddome/ome),dabs(ddber/ber))
!    numero = numero + 1
!    if (numero<1000) then
...
!    if (dabs(error)<1.0d-08) exit
    if (dabs(error)<1.0d-12) then
      write (6,'(a22, 1p,3e20.12)') " New ome, ber, radi = ", ome, ber, radi
      exit
    end if
    numero = numero + 1
  end do
...
!  radi = convf*radi + (1.d0-convf)*radiold
  write (6,'(a22, 1p,3e20.12)') " Upd ome, ber, radi = ", ome, ber, radi
!
!
