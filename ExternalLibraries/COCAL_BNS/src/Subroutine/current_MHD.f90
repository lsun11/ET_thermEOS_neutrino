subroutine current_MHD
  ...
  real(long) :: alphff, psiff, alps6f, jphiuf
  integer    :: irf, itf, ipf
!
...
!
      jphiuf = 1.0d0/alps6f &
      &  *((MHDfnc_d2Psi*huphi + MHDfnc_dLambda_phi)*Bphi &
      &   - MHDfnc_dPSI*vorphi) &
      &   -(MHDfnc_d2At*huphi + MHDfnc_dLambda)*rho*ut
      jyuf(irf,itf,ipf) = jphiuf*vecphif(2)
!
! jt 
      jtuf(irf,itf,ipf) = 0.0d0
!      jtuf(irf,itf,ipf) = - 1.0d0/MHDfnc_dAt*MHDfnc_d2At*huphi*rho*ut
!
    end do
  end do
!
