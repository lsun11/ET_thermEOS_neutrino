...
! value inside of the companion BH is set to zero in interpo_gr2gr_4th(_mpt)
        ...
        if (alpgc.le.0.0d0) alpgc = 1.0d-10
...
!
  do ipg = 1, npg
    do itg = 1, ntg
      do irg = 1, nrg
        call grgrad_midpoint_r4th_type0(divBeta,dxafn,dyafn,dzafn, &
        &                                       irg,itg,ipg,'bh')
        ddivBeta(irg,itg,ipg,1) = dxafn        
        ddivBeta(irg,itg,ipg,2) = dyafn
        ddivBeta(irg,itg,ipg,3) = dzafn
        call grgrad_midpoint_r4th_type0(fnc2,dxafn,dyafn,dzafn, &
        &                                    irg,itg,ipg,'bh')
        grad2x(irg,itg,ipg) = dxafn
        grad2y(irg,itg,ipg) = dyafn
        grad2z(irg,itg,ipg) = dzafn
      end do
    end do
  end do
!
