...
!     print *, "INSIDE  vol_integrands-movingbox.F90, track_bhns=",track_bhns
  ...
  if(mod(cctk_iteration,VolIntegral_out_every)==0 .and. track_bhns.eq.0) then
...
!$omp end parallel do
  ...
  else if(mod(cctk_iteration,VolIntegral_out_every)==0 .and. cctk_iteration .gt. 1 .and. track_bhns.eq.1) then
     !-----------------------------------------------------------------------------
     ! Compute integrands
     !-----------------------------------------------------------------------------
     !Reset VolIntegrand's quickly, using OpenMP
