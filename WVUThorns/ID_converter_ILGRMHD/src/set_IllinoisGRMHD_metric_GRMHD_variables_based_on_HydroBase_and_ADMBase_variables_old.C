...
extern "C" void set_IllinoisGRMHD_metric_GRMHD_variables_based_on_HydroBase_and_ADMBase_variables(CCTK_ARGUMENTS) {
  ...
  eos.neos=neos;
  eos.K_poly=K_poly;
  eos.ergo_star=ergo_star;
  eos.ergo_sigma=ergo_sigma;
  //Setup eos struct 
  for(int nn=0;nn<neos+ergo_star;++nn){
     eos.k_tab[nn]=k_tab[nn];
     eos.rho_tab[nn]=rho_tab[nn];
     eos.gamma_tab[nn]=gamma_tab[nn];
     eos.eps_tab[nn]=eps_tab[nn];
     eos.P_tab[nn]=P_tab[nn];
  }
  */
...
#pragma omp parallel for
  for(int k=0;k<cctk_lsh[2];k++) for(int j=0;j<cctk_lsh[1];j++) for(int i=0;i<cctk_lsh[0];i++) {
	...
	if(recompute_pressure){
		...
		P[index]=Pressure_cold_rho(rho_b[index],eos,eps[index]);
	}
	...
	if(check_gamma_th){
		...
		if(rho_b[index]>rho_b_atm && fabs(gamma_th - measured_gamma)/gamma_th > 1e-2)
	  	CCTK_VError(VERR_DEF_PARAMS,
                	      "Expected simple gamma law with gamma_th=%.15e, but found a point with gamma law such that gamma_th=%.15e. error = %e| rb=%e rbatm=%e\n",
                      	       gamma_th,measured_gamma,(gamma_th-measured_gamma)/gamma_th,rho_b[index],rho_b_atm );
	}
